...
int main() {
    ...
    ofstream bifuOut("bifu_plot.txt");
    
    for(int igam=0; igam < 250; igam++){
        ...
        if (plotTrajectory) {
            ...
            double temp = 500.0, difference = 0.0;
            
            for( i=0; i< t_plot.size(); i++ ) {
                
                if(t_plot[i] >= 601) break;
                difference = t_plot[i] - temp;
                if(difference >= 1){
                    bifuOut << gamma << " "  << th_plot[i] << endl;
                    temp = t_plot[i];
                }
