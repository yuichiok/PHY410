...
double diff( std::vector<double>& f, double interval, int x){
    
    double dfdx=0;
    double h=0;
    h = interval;
    if (x >= 2 && x < f.size()-2) {
        dfdx = ( f[x-2] - 8*f[x-1] + 8*f[x+1] - f[x+2]) / (12*h);
    }else if(x == 1||x == f.size()-2){
        dfdx = (f[x+1] - f[x-1]) / (2*h);
    }else if(x == 0){
        dfdx = (f[x+1] - f[x]) / h;
    }else if(x == f.size()-1){
        dfdx = (f[x] - f[x-1]) / h;
...
int main()
{
    ...
    double E = 0.70541;      // set global value of E
    cout << " Energy E = " << E << endl;
    double b_min = 1.70, db = 0.01;
    int n_b = 21;
    double b = 0.0;
    double V0 = 1.0;
    std::vector<double> vecb, vecangle;
    ...
    for (int i = 0; i < n_b; i++) {
        ...
        sstream << "./data/trajfile_cpp_" << i << ".data";
        ...
        vecb.push_back(b);
        while (std::abs(deflection) > 2*M_PI) {
            deflection = std::abs(deflection) - 2*M_PI;
        }
        vecangle.push_back(std::abs(deflection));
    ...
    for (int j=0; j < vecb.size(); j++) {
        ...
        dt_db = diff(vecangle, db, j);
        dSig_dO = vecb[j]/(sin(vecangle[j])*std::abs(dt_db));
        //std::cout << "diff cross = " << dSig_dO << std::endl;
        char test[1000];
        sprintf(test, "%8.4f %8.4f", vecb[j], dSig_dO);
        file2 << test << std::endl;
    }
    file2 << std::endl;
